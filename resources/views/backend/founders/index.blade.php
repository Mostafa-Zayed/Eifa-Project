...
@section('page-header')
    <div class="page-title">
        <div class="row">
            <div class="col-sm-6">
                <h4 class="mb-3">
                    <button type="button" class="button x-small" data-toggle="modal" data-target="#addFounderModal">
                        {{ucwords(trans('dashboard.add_founder'))}}
                    </button>
                </h4>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                    ...
                    <li class="breadcrumb-item active">{{trans('dashboard.all_founders')}}</li>
                </ol>
            </div>
        </div>
    </div>
...
@section('content')
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="datatable" class="table table-striped table-bordered p-0">
                            <thead>
                            <tr>
                                <th>{{ucwords(trans('founders.image'))}}</th>
                                <th>{{ucwords(trans('founders.name'))}}</th>
                                <th>{{ucwords(trans('founders.description'))}}</th>
                            ...
                            @if(isset($founders) && $founders->count() > 0)
                                @foreach($founders as $founder)
                                    <tr>
                                        ...
                                        <td>{{$founder->order}}</td>
                                        <td>
                                            @if(! empty($founder->status) && $founder->status == 1)
                                                <button type="button" class="btn btn-danger" data-id="{{$founder->id}}">Not Active</button>
                                            @else
                                                <button type="button" class="btn btn-success" data-id="{{$founder->id}}">Active</button>
                                            @endif
                                        </td>
                                        <td>
                                            <a class="btn btn-warning" href="{{route('founders.edit',$founder->id)}}">Edit</a>
                                        </td>
                                        <td>
    ...
    <!-- row closed -->
    <div class="modal fade" id="addFounderModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 style="font-family: 'Cairo', sans-serif;" class="modal-title" id="exampleModalLabel">{{ucwords(trans('dashboard.add_founder'))}}</h5>
                ...
                <form action="{{route('founders.store')}}" method="POST">
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col">
                                <label for="name_ar" class="mr-sm-2">{{ucwords(trans('founders.name_ar'))}} :</label>
                            ...
                            <div class="col">
                                <label for="name_en" class="mr-sm-2">{{ ucwords(trans('founders.name_en')) }}   :</label>
    ...
    <script>
        function openModelWithErrors() {
            ...
            if (isSessionHasError == 1) {
                let addPageModal = $('#addFounderModal');
